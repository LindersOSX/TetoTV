...
Public `1.0.1` and the private test build Beta `2.0.4` use the same Android
...
Beta started at `v2.0.0`; the current private test build is tagged `v2.0.4` and
its user-facing release name is **TetoTV 2.0.4 Beta**. Keep machine-readable
tags strictly numeric (`v2.0.4`); the app adds the `Beta` channel label in its UI.
...
```json
{"releases":[{"version":"2.0.4","tag_name":"v2.0.4","asset":{"download_url":"https://tetotv-updates-lindows.onrender.com/v1/app-updates/releases/v2.0.4/assets/123/universal.apk"}}]}
```
